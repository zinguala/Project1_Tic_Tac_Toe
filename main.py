...
def win_check(args, player_help=0):  # function for win check and option to help the player.
    ...
    if player_help == 1 and count_second_diagonal_x == (count_for_win - 1):
        for r in range(len(args)):   # second diagonal check for the
            if args[count_for_win - r - 1][r] == '_':  # check the second diagonal for the
                print(f'Player 2 next move for win choose square: {count_for_win - r - 1}{r}')

    if player_help == 2 and count_second_diagonal_o == (count_for_win - 1):
        for n in range(len(args)):   # second diagonal check for the
            if args[count_for_win - n - 1][n] == '_':  # check the second diagonal for the
